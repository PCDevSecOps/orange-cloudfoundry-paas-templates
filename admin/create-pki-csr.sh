#!/bin/bash
#===========================================================================
# Create private key and CSR for PKI certs
#===========================================================================
...
while [ ${flag} = 0 ] ; do
  ...
  printf "%b6%b : DNS4CLOUD cert\n" "${GREEN}${BOLD}" "${STD}"
  printf "%bq%b : Quit\n" "${GREEN}${BOLD}" "${STD}"
  printf "\n%bYour choice :%b " "${GREEN}${BOLD}" "${STD}" ; read choice
  case "${choice}" in
    1) CERT_DIR="api-certs" ; CERT_DOMAIN="/secrets/cloudfoundry/system_domain" ; flag=1 ;;
    2) CERT_DIR="intranet-1-certs" ; CERT_DOMAIN="/secrets/intranet_interco_1/apps_domain" ; flag=1 ;;
    3) CERT_DIR="intranet-2-certs" ; CERT_DOMAIN="/secrets/intranet_interco_2/apps_domain" ; flag=1 ;;
    4) CERT_DIR="ops-certs" ; CERT_DOMAIN="/secrets/ops_interco/ops_domain" ; flag=1 ;;
    5) CERT_DIR="osb-certs" ; CERT_DOMAIN="/secrets/osb_interco/osb_domain" ; flag=1 ;;
    6) CERT_DIR="dnsaas-certs" ; CERT_DOMAIN="/secrets/multi_region/region_1/dnsaas/rfc2136_zone_backend_services" ; flag=1 ;;
    *) printf "\n" ; exit 1 ;;
  esac
done
...
commitGit "${SHARED_CERT_DIR}" "update_pki_csr"
printf "\n"