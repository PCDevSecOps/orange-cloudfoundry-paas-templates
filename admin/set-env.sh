...
#--- Set proxy variables
export PROXY_URL="http://10.228.194.12:443"
...
flag="$(grep "PATH=" ~/.bashrc | grep "${HOME}/bin:${HOME}/.rvm/rubies/ruby-${RUBY_VERSION}/bin")"
if [ "${flag}" = "" ] ; then
  display "INFO" "Set PATH"
  sed -i -e "/export PATH=.*/d" ${HOME}/.bashrc
  printf "\nexport PATH=.:${HOME}/bin:${HOME}/.rvm/rubies/ruby-${RUBY_VERSION}/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:${HOME}/.rvm/bin\n\n" >> ${HOME}/.bashrc
fi
...
alias micro-bosh='ssh vcap@192.168.10.10 -i ${HOME}/bosh/secrets/shared/keypair/bosh.pem'
alias inception='ssh inception@192.168.10.2 -i ${HOME}/bosh/secrets/bootstrap/inception/inception.pem'
EOF
