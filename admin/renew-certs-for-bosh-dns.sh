...
generateCredhubCert() {
  ...
  if [ "$1" = "CA" ] ; then
    ...
    credhub generate -t certificate -n ${PATH_NAME} -c ${COMMON_NAME} -a ${COMMON_NAME} -a *.${COMMON_NAME} --ca=$4 -e $5 > /dev/null 2>&1
    if [ $? != 0 ] ; then
      printf "\n%bERROR: \"${PATH_NAME}\" certificate creation failed.%b\n\n" "${RED}" "${STD}" ; exit 1
    fi
  fi
...
commitGit "${DNS_CERT_DIR}" "update_bosh-dns_certs"
