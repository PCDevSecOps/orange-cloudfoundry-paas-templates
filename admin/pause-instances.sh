...
#--- Bosh directors (not stopped)
BOSH_DIRECTORS="micro master ops coab remote-r2 remote-r3"

#--- These deployments must not be stopped (needed to restart)
DO_NOT_STOP_DEPLS="cfcr cfcr-persistent-worker credhub-ha dns-recursor docker-bosh-cli intranet-interco-relay internet-proxy jcr nexus openldap ops-routing logsearch logsearch-ops io-bench"

#--- These deployments (on master-depls) will be stopped at the end and restart before the others (required by other deployments)
PRE_START_DEPLS="cf weave-scope"

#--- Script parameters
SHARED_SECRETS="${HOME}/bosh/secrets/shared/secrets.yml"
INTERNAL_CA_CERT="/etc/ssl/certs/ca-certificates.crt"

#--- Don't operate on special deployments that need to stay active (including bosh directors)
BOSH_DIRECTORS_DEPLS="$(echo " ${BOSH_DIRECTORS}" | sed -e "s+ + bosh-+g" | sed -e "s+^ ++")"
NO_ACTION_DEPLS=$(echo "${BOSH_DIRECTORS_DEPLS} ${DO_NOT_STOP_DEPLS} ${PRE_START_DEPLS}" | sed -e "s+ + | +g")
...
operateDeployments() {
  ...
  if [ "${flag}" != "" ] ; then
    printf "\n%b$(date) : ${OPERATION} deployments on ${BOSH_TARGET}-depls%b\n" "${REVERSE}${YELLOW}" "${STD}" | tee -a ${LOG_FILE}
      ...
      if [ $? = 0 ] ; then
        ...
        if [ "${DEPLOYMENTS}" != "" ] ; then
          ...
          if [ ${nohup_deployments} -gt 0 ] ; then
            ...
            printf "\n"
            while [ ${loop} = 1 ] ; do
              nb_todo=$(ps -ef | grep "bosh -n ${OPERATION}" | grep -v "grep" | wc -l)
          ...
          if [ ${checkOperation} = 1 ] ; then
            printf "\n%b$(date) : check ${OPERATION} deployments on \"${BOSH_TARGET}-depls\"...%b\n" "${REVERSE}${YELLOW}" "${STD}" | tee -a ${LOG_FILE}
            for BOSH_DEPLOYMENT in ${DEPLOYMENTS} ; do
              export BOSH_DEPLOYMENT
...
if [ ! -s "${SHARED_SECRETS}" ] ; then
  printf "\n%bERROR: Credentials file \"${SHARED_SECRETS}\" unknown.%b\n\n" "${REVERSE}${RED}" "${STD}" ; exit 1
fi
