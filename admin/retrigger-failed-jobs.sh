...
all_unsuccessful_jobs=$($FLY_BIN -t "${FLY_TARGET}" curl /api/v1/jobs -- -s -S -L|jq --arg fly_target int '.[]?|select(has("paused")|not)|.finished_build|select(.status != "succeeded")|select(values)| {"pipeline": .pipeline_name, "cmd": "fly -t \($fly_target) tj --team \(.team_name) -j \(.pipeline_name)/\(.job_name)"}')
