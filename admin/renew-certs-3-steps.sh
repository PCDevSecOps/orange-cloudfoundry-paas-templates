...
renewCACerts() {
  ...
  if [ "${next}" = "ko" ] ; then
    printf "\n%bERROR: Cannot use this script for \"${BOSH_DEPLOYMENT}\" certs rotation.%b\n\n" "${RED}" "${STD}" ; exit 1
  fi
  ...
  printf "\n%bRenew \"/${BOSH_DEPLS_NAME}/${BOSH_DEPLOYMENT}\" certs (y/[n]) ? :%b " "${REVERSE}${GREEN}" "${STD}" ; read choice ; printf "\n"
  if [ "${choice}" != "y" ] ; then
    exit 1
  fi
  ...
  executeCredhubCurl "PUT" "data" \"name\":\"/certs_rotation_next_step\",\"type\":\"value\",\"value\":\"2\"
...
renewLeafCerts() {
  ...
  printf "\n%bHave you redeploy all linked deployments (to propagate new CA certs)%b\n\n%bContinue (y/[n]) ? :%b " "${BLINK}${REVERSE}${GREEN}" "${STD}" "${REVERSE}${GREEN}" "${STD}" ; read choice ; printf "\n"
  if [ "${choice}" != "y" ] ; then
    exit 1
  fi
  ...
  if [ "${DEPLOYMENT_CA_CERT_NAMES}" = "" ] ; then
    ...
    executeCredhubCurl "no_chek_errors" "DELETE" "data?name=/certs_rotation_next_step"
    ...
    executeCredhubCurl "no_chek_errors" "DELETE" "data?name=/certs_rotation_next_step"
    executeCredhubCurl "PUT" "data" \"name\":\"/certs_rotation_next_step\",\"type\":\"value\",\"value\":\"3\"
...
executeCredhubCurl "no_chek_errors" "GET" "data?name=/certs_rotation_next_step"
