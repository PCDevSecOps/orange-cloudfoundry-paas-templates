...
JUMPBOX_PEM_FILE="${SECRETS_REPO_DIR}/shared/keypair/jumpbox.pem"
...
if [ ${FLAG_CONCOURSE} = 1 ] ; then
  #--- Concourse worker container
  BOSH_PERSISTENT_DIR="/root/.bosh"

  if [ -d ${BOSH_PERSISTENT_DIR} ] ; then
    rm -fr ${BOSH_PERSISTENT_DIR} > /dev/null 2>&1
  fi

  mkdir -p ${BOSH_PERSISTENT_DIR}/downloads > /dev/null 2>&1
  chmod 777 ${BOSH_PERSISTENT_DIR} ${BOSH_PERSISTENT_DIR}/downloads
else
  ...
  if [ ! -d ${BOSH_PERSISTENT_DIR}/downloads ] ; then
    sudo mkdir -p ${BOSH_PERSISTENT_DIR}/downloads
  fi
  if [ ! -d ${BOSH_PERSISTENT_DIR}/installations ] ; then
    sudo mkdir -p ${BOSH_PERSISTENT_DIR}/installations
  fi
  sudo chmod 777 ${BOSH_PERSISTENT_DIR} ${BOSH_PERSISTENT_DIR}/downloads ${BOSH_PERSISTENT_DIR}/installations
  ...
  rm -fr ~/.bosh/downloads ~/.bosh/installations > /dev/null 2>&1
  ln -s ${BOSH_PERSISTENT_DIR}/downloads ~/.bosh/downloads
  ln -s ${BOSH_PERSISTENT_DIR}/installations ~/.bosh/installations
...
#--- Update "default_ca" with internalCA cert (keep private key)
...
if [ ${FLAG_RECREATE_CERTS} = 1 ] ; then
  ...
  cleanCert "${MICRO_BOSH_CREDENTIALS}" "default_ca"
...
spruce merge --prune secrets ${SHARED_SECRETS} template/micro-bosh-vars-tpl.yml template/dns-recreate-vars-tpl.yml template/${IAAS_TYPE}/${IAAS_TYPE}-vars-tpl.yml > micro-bosh-vars.yml
if [ $? != 0 ] ; then
  display "ERROR" "Generate micro-bosh vars failed"
fi

#--- Get ssh public key from private key
chmod 600 ${BOSH_PEM_FILE}
...
bosh int bosh-deployment/bosh.yml \
  ...
  -o template/dns-recreate-operators.yml \
...
#--- Set dns from secrets.yml
if [ -f ${MICRO_BOSH_SECRETS} ] ; then
  display "INFO" "Update dns in micro-bosh manifest"  
  ...
  if [ $? != 0 ] ; then
    display "ERROR" "Update dns in micro-bosh manifest failed"
  fi
fi
...
chmod 600 ${BOSH_PEM_FILE} ${JUMPBOX_PEM_FILE} > /dev/null 2>&1
