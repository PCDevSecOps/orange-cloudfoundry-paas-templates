...
for bosh_director in ${BOSH_DIRECTORS} ; do
  ...
  if [ "${status}" != "" ] ; then
    ...
    if [ "${status}" != "" ] ; then
      ...
      EXCLUDED_JOBS="cloud-config-and-runtime-config-for-${bosh_director}-depls execute-deploy-script init-concourse-boshrelease-and-stemcell-for-${bosh_director}-depls ${EXCLUDED_JOBS}"
