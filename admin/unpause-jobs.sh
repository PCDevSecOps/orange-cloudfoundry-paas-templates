...
unpauseJobs() {
  ...
  if [ "${status}" != "" ] ; then
    ...
    EXCLUDED_JOBS="deploy-nexus deploy-jcr deploy-internet-proxy deploy-gitlab deploy-concourse deploy-credhub-ha ${EXCLUDED_JOBS}"
...
if [ "${PIPELINES}" != "" ] ; then
  ...
  do
    ...
    case "${choice}" in
      ...
      4) unpauseJobs "coab" ;;
      5) unpauseJobs "kubo" ;;
      6) unpauseJobs "remote-r2" ;;
      7) unpauseJobs "remote-r3" ;;
      q) printf "\n" ; exit 1 ;;
    esac
  done
fi
