...
unpauseJobs() {
  ...
  if [ "${status}" != "" ] ; then
    ...
    EXCLUDED_JOBS="deploy-nexus deploy-jcr deploy-internet-proxy deploy-gitlab deploy-concourse deploy-credhub-ha deploy-r1-vpn deploy-00-bootstrap ${EXCLUDED_JOBS}"
