...
#wget -O /usr/local/bin/bosh "https://s3.amazonaws.com/bosh-cli-artifacts/bosh-cli-${BOSH_CLI_VERSION}-linux-amd64"
...
#--- Install minio
printf "%bInstall minio cli \"${CREDHUB_CLI_VERSION}\"...%b\n" "${YELLOW}" "${STD}"
rm -f mc
#(wget "https://dl.minio.io/client/mc/release/linux-amd64/mc" -O /usr/local/bin/mc 2>&1 ; echo $? > ${STATUS_FILE})
