...
building_third_level(){
    ...
    profile=$3
    service=$4
    >/tmp/service_third_level_${service}.lst
    for file in $(find ${model}/${template}/${profile} -maxdepth 1 -type f -name '*.yml'); do
        #../../../cf-mysql/template/vsphere/99-osb-operators.yml
        relative=$(echo ${file} | awk -v var=${root_path} '{gsub(var,"") ; print}')
        echo "../../..${relative}" >> /tmp/service_third_level_${service}.lst
    done
    cat /tmp/service_third_level_${service}.lst
...
migrating_third_level(){
    ...
    #linking
    for line in $(cat /tmp/service_third_level_${service}.lst); do
        #echo "ln -s ${line} $(basename ${line})"
        ln -s ${line} $(basename ${line})
    done
...
migrating() {
    ...
    #root_path=$(pwd)"/paas-templates/coab-depls"
    echo ${rp}
    cd ${rp}
