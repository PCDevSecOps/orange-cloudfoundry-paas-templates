...
# COA-provided: $SECRETS_DIR (present in both pre-cf-push and post-deplo)
# $1: property name relative to /secrets/
# $2: default value
...
function fetch_deployment_secret_prop() {
    ...
    ! bosh int "${SECRETS_DIR}/secrets/secrets.yml" --path /secrets/${PROPERTY_NAME} >/tmp/fetch_deployment_secret_value.txt 2> /tmp/fetch_deployment_secret_prop.txt
    ...
    if [[ ${NEGATED_EXIT_CODE} -eq 0 ]]; then
        if [[ -n "${DEFAULT_VALUE}" ]]; then
            ...
            >&2 echo "Unable to resolve mandatory property \"/secrets/${PROPERTY_NAME}\" in ${SECRETS_DIR}/secrets/secrets.yml. bosh int failed with message: $(cat /tmp/fetch_deployment_secret_prop.txt)"
            false
        fi
    else
