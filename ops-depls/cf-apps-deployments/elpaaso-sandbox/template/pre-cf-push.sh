...
SB_SERVICE_GITHUB_BRANCH=master





echo "downloading Sandbox service binary with manifest reference"
curl -L -s  ${SB_SERVICE_URL} -o ${GENERATE_DIR}/elpaaso-sandbox-service.jar
curl -L -s https://raw.githubusercontent.com/Orange-OpenSource/elpaaso-sandbox-service/$SB_SERVICE_GITHUB_BRANCH/cf-manifest-reference.yml  -o ${GENERATE_DIR}/cf-manifest-reference.yml

echo "downloading Sandbox ui binary WITHOUT manifest reference"
