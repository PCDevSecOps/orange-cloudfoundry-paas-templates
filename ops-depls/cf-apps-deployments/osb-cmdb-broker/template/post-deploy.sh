...
# $1: service instance name
# $2: nb expected service instances (exact equality expected)
# $3: service instance guid (optional). Useful when the service instance does not exist anymore. Otherwise loaded from `cf service $1 --guid`
function assert_service_instance_number() {
  ...
  local NB_EXPECTED_INSTANCES="$2"
  local SERVICE_INSTANCE_GUID="$3"
  ...
  if [[ ${NB_BACKING_SERVICES} -ne ${NB_EXPECTED_INSTANCES} ]]; then
    echo "expected ${NB_EXPECTED_INSTANCES} backing service instances matching guid=${SERVICE_INSTANCE_GUID} but got ${NB_BACKING_SERVICES}"
...
# $1: service instance name
# $2: service instance guid
function assert_delete_service_instance() {
  assert_service_instance_number "$1" "0" "$2"
}
