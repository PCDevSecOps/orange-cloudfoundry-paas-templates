...
#osb-cmdb specifics
export REPO_NAME=osb-cmdb-spike
...
function generate_credhub_var_password_request() {
...
generate_credhub_var_password_request


#Download osb-cmdb.jar, generate credhub variable
${BASE_TEMPLATE_DIR}/common-pre-cf-push.sh

#Replace application.yml from jar with content specified in secrets.yml
...
printf "%s" "${APPLICATION_YML}" > BOOT-INF/classes/application.yml

apk add zip
zip -r  ${GENERATE_DIR}/${JAR_ARTEFACT_BASE_NAME}.jar BOOT-INF/classes/application.yml
#check replacement was correctly done
unzip -l ${GENERATE_DIR}/${JAR_ARTEFACT_BASE_NAME}.jar BOOT-INF/classes/application.yml
echo "broker configured with application.yml content:"
ls -al BOOT-INF/classes/application.yml
