...
## feature-bump-cf-deployment-13.20.0
...
- ## How to review a cleaned instance branch (for example cf-rabbit service) ?  
    - go to secrets repository and execute the command
    ```bash
    find ./coab-depls -type d -name "r_*" > /tmp/secrets.lst
    ``` 
    - go to paas-templates repository, checkout the  and execute the command
    ```bash
    find ./coab-depls -type d -name "r_*" > /tmp/paas-templates.lst
    ``` 
    - compare the two files and check that there is no difference
    ```bash
