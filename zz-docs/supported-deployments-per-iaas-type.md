...
| 51-switch-to-k8s-traefik            | ops relay to 00-core-connectivity |
| 51-switch-to-k8s-gitlab             | new url, COA change, cf apps,coab) | 
| 51-switch-to-k8s-jcr-for-docker-k3s | docker release and k3s mirrors config). recreate required to check new url usage (+bosh stop micro-depls/jcr)|
| 51-switch-to-k8s-minio              | change COA, nouvelle url, reimporter les datas avec le errand) |
| 51-switch-to-k8s-openldap           | dns switch
| 51-switch-internet-proxy            | dns switch - FE floating ip, vsphere FPV/fe-int) |
| 51-switch-intranet-proxy           |  dns switch change de dns devops store)|
...
|k8s-minio| X | X | 
|k8s-openldap| X | X |
|k8s-traefik-core-connectivity| X | X |
