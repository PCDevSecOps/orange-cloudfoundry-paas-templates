...
VARS_FILES="$(find -L * -name "*-vars.yml")"
...
MANIFESTS="$(find -L * -name "*.yml" -o -name "*.yaml" )"
...
for file in ${MANIFESTS} ; do
  ...
  if grep -q '((' "${file}"; then
   ...
   #raw copy
   cp -f -L ${file} ${K8S_GIT_REPO_PATH}/${COA_ROOT_DEPLOYMENT_NAME}/${COA_DEPLOYMENT_NAME}/k8s-config/manifests/${file}
