...
echo "======================================="
printf "%bInstalling version ${PAAS_TEMPLATES_VERSION} located in Paas templates dir: '${PAAS_TEMPLATES_DIR}'%b\n" "${GREEN}${BOLD}" "${STD}"
printf "%busing secrets dir at '${SECRETS_DIR}'%b\n" "${GREEN}${BOLD}" "${STD}"
...
if ! git fetch automated-install 1>/dev/null 2>&1; then
  printf "%bPlease check git url. '${LOCAL_GITLAB_URL}' is invalid %b\n" "${RED}${BOLD}" "${STD}"
  exit 1
fi
