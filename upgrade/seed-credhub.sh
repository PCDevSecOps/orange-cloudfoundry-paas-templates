...
if [ -z "$PT_REFERENCE_BRANCH" ]; then
  ...
  echo "Extracting 'PT_REFERENCE_BRANCH' from '${SECRETS_DIR}/coa/config/credentials-sync-feature-branches-pipeline.yml'"
fi
...
if [ -z "$STEMCELL_MICRO" ]; then
  ...
  echo "Extracting 'STEMCELL_MICRO' from '${PAAS_TEMPLATES_DIR}/micro-depls/root-deployment.yml'"
fi
...
if [ -z "$CF_BOSH_CLI_VERSION" ]; then
  ...
  echo "Extracting 'CF_BOSH_CLI_VERSION' from '${PAAS_TEMPLATES_DIR}/upgrade/metadata.yml'"
fi


CREDHUB_CLIENT_SECRET=$(credhub-get /secrets/bosh_credhub_secrets)
echo "Extracting 'CREDHUB_CLIENT_SECRET' from credhub '/secrets/bosh_credhub_secrets'"
IAAS_TYPE=$(credhub-get /secrets/iaas_type)
echo "Extracting 'IAAS_TYPE' from credhub '/secrets/iaas_type'"
