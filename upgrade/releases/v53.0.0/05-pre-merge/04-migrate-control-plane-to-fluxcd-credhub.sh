...
SetCredhubMissingValue() {
  ...
  credhub set -t ${TYPE} -n "${PATH_NAME}" ${VALUE_OPTION} "${VALUE}" > /dev/null 2>&1
  if [ $? != 0 ] ; then
...
DuplicateKeyTypePassword(){
  ...
  already_migrated="$(credhub get -q -n $new_key 2>/dev/null)"
  if [ -n "$new_key" ];then
    echo "Skipping migration: $new_key already migrated"
    return
  fi
