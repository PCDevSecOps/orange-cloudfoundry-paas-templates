...
if [ "$searched_job" = "$job_name" ]; then
  echo "Interacting with pipeline ${CONCOURSE_URL}/teams/${team}/pipelines/${pipeline}/${pipeline}/jobs/${job}"
  ${FLY} -t "${FLY_TARGET}" unpause-job -j "${pipeline}/${job_name}"
else
  echo "Info: pipeline does not exist: ${CONCOURSE_URL}/teams/${team}/pipelines/${pipeline}/${pipeline}/jobs/${job}"
fi