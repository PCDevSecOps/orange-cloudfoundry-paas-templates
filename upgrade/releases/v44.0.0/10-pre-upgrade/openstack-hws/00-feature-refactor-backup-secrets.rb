...
def feature_refactor_backup_secrets(secrets_dir)
  ...
  s3_host = shared_secrets_yaml.dig("secrets","shield","s3_host")
  s3_access_key_id = shared_secrets_yaml.dig("secrets","shield","s3_access_key_id")
  s3_secret_access_key = shared_secrets_yaml.dig("secrets","shield","s3_secret_access_key")
  s3_bucket_prefix = shared_secrets_yaml.dig("secrets","shield","s3_bucket_prefix")
  ...
  unless s3_bucket_prefix.nil?
    ...
    puts "s3_bucket_prefix already migrated"
  end
  ...
  unless s3_access_key_id.nil?
    ...
    puts "s3_access_key_id already migrated"
  end
  ...
  unless s3_secret_access_key.nil?
    ...
    puts "s3_secret_access_key already migrated"
  end
