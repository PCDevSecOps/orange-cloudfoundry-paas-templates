...
if version == "latest"
  puts "'latest' version detected, selecting latest version available"
  version = display_available_versions.last
  puts "selected upgrade scripts from '#{version}'"
end
...
puts("ensure all scripts are executable")
Dir.chdir(upgrade_dir) do
  FileUtils.chmod "a+x", Dir['**/*.sh'] + Dir['**/*.rb'], verbose: true
end
