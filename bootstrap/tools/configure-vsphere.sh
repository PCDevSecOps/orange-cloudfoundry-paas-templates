...
Step_2.1() {
  ...
  TEAMS="main micro-depls master-depls ops-depls coab-depls remote-r2-depls remote-r3-depls utils"
...
Step_2.2() {
  ...
  MINIO_S3_KEY=$(getCredhubValue "/micro-bosh/01-ci-k8s/minio-secret-key")
...
Step_6() {
  ...
  updateYaml ${SHARED_SECRETS} secrets.coa.config.docker-registry-url "docker.jcr-k8s.${OPS_DOMAIN}/"
  ...
  for team in ${TEAMS} ; do
    credhub d -n /concourse-micro/${team}/docker-registry-url > /dev/null 2>&1
    credhub s -t value -n /concourse-micro/${team}/docker-registry-url -v "docker.jcr-k8s.${OPS_DOMAIN}/" > /dev/null 2>&1
    if [ $? != 0 ] ; then
