...
Step_1() {
  ...
  TEMPLATE_CERT_FILE="${INCEPTION_BOOTSTRAP_DIR}/template/tpl-certs.yml"
  ...
  updateYaml ${SHARED_SECRETS} secrets.bosh.root.password_to_use ${VCAP_PASSWORD}
  ...
  updateYaml ${SHARED_SECRETS} secrets.shield.autoprovision_key "$(setPassword)"
  updateYaml ${SHARED_SECRETS} secrets.shield.basic_password "$(setPassword)"
  updateYaml ${SHARED_SECRETS} secrets.shield.obos_secret_access_key "$(setPassword)"
  ...
  updateYaml ${SHARED_SECRETS} secrets.cloudfoundry.service_brokers.coa-cassandra-broker.password "$(setPassword)"
  ...
  updateYaml ${SHARED_SECRETS} secrets.cloudfoundry.service_brokers.coa-noop-broker.password "$(setPassword)"
  ...
  updateYaml ${SHARED_SECRETS} secrets.networks.net-bosh-2 $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_bosh_2)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf_net_exchange $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_exchange)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-osb-control-plane $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_osb_control_plane)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-osb-data-plane-shared-pub $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_osb_data_plane_shared_pub)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-osb-data-plane-shared-pub2 $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_osb_data_plane_shared_pub2)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-osb-data-plane-shared-priv $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_osb_data_plane_shared_priv)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-osb-data-plane-dedicated-priv $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_osb_data_plane_dedicated_priv)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-cf $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_cf)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-cfcr-master $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_cfcr_master)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-cfcr-micro $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_cfcr_micro)
  updateYaml ${SHARED_SECRETS} secrets.networks.tf-net-kubo $(getValue ${BOOTSTRAP_VARS_FILE} /networks/net_cfcr_kubo)
  ...
  display "INFO" "Update osb data plane shared public 2 network"
  ...
  display "INFO" "Update S3 shield credentials"
  updateYaml ${SHARED_SECRETS} secrets.shield.s3_host $(getValue ${BOOTSTRAP_VARS_FILE} /shield/s3_host)
  updateYaml ${SHARED_SECRETS} secrets.shield.obos_access_key_id $(getValue ${BOOTSTRAP_VARS_FILE} /shield/s3_access_key_id)
  updateYaml ${SHARED_SECRETS} secrets.shield.obos_bucket_prefix $(getValue ${BOOTSTRAP_VARS_FILE} /shield/s3_bucket_prefix)
  ...
  display "INFO" "Update credhub configuration"
  ...
  display "INFO" "Update coa configuration"
  ...
  updateYaml ${COA_CONFIG_DIR}/credentials-kubo-depls-bosh-pipeline.yml bosh-password ${BOSH_ADMIN_PASSWORD}

  updateYaml ${COA_CONFIG_DIR}/credentials-docker-registry.yml docker-registry-url "\"\""
  ...
  display "INFO" "Set coa in online mode"
...
#--- Configuration step after bosh-dns certs creation
Step_2.1() {
...
Step_2.2() {
  ...
  display "INFO" "Set shield obos secret access key to credhub"
  credhub s -t value -n /secrets/shield_obos_secret_access_key -v $(getValue ${SHARED_SECRETS} /secrets/shield/obos_secret_access_key) > /dev/null 2>&1
  if [ $? != 0 ] ; then
    display "ERROR" "Set propertie to credhub failed"
  fi
  ...
  updateYaml ${COA_CONFIG_DIR}/credentials-auto-init.yml concourse-kubo-depls-username ${CONCOURSE_USER}
  updateYaml ${COA_CONFIG_DIR}/credentials-auto-init.yml concourse-kubo-depls-password ${CONCOURSE_PASSWORD}
  updateYaml ${COA_CONFIG_DIR}/credentials-auto-init.yml concourse-kubo-depls-target ${CONCOURSE_ENDPOINT}
...
Step_5() {
  ...
  display "INFO" "Enable \"intranet-interco-relay\" proxy operators"
...
Step_7() {
  ...
  display "INFO" "Enable \"nexus\" registry operators"
  ...
  display "INFO" "Set \"nexus\" registry for concourse workers"
  OPS_DOMAIN=$(getValue ${SHARED_SECRETS} /secrets/ops_interco/ops_domain)
  updateYaml ${COA_CONFIG_DIR}/credentials-docker-registry.yml docker-registry-url "elpaaso-nexus.${OPS_DOMAIN}/"

  commitGit "template" "enable_nexus_registry"
}
...
Step_9.2() {
  ...
  updateYaml ${COA_CONFIG_DIR}/credentials-auto-init.yml concourse-kubo-depls-target https://elpaaso-concourse.${OPS_DOMAIN}
