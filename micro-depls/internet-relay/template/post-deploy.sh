...
ADMIN_PASSWORD_PATH="/secrets/bosh_admin_password"
export BOSH_ENVIRONMENT="192.168.10.10"
export BOSH_DEPLOYMENT="internet-relay"
...
printf "%bInstall openssh cli...%b\n" "${YELLOW}" "${STD}"
apk update
apk add --no-cache --no-progress openssh
if [ $? != 0 ] ; then
	printf "\n%bERROR: Install openssh-client failed%b\n\n" "${RED}" "${STD}" ; exit 0
fi
