...
if [ "${IAAS_TYPE}" = "vsphere" ] ; then
  OPERATORS_FILES="-o ../template/1-use-http-proxy-for-garden-docker-operators.yml \
  -o ../template/02-create-local-user-into-credhub-operators.yml \
  -o ../template/03-add-bosh-dns-requirements-on-worker-operators.yml \
  -o ../template/04-add-vm-extension-operators.yml \
  -o ../template/04-customize-garden-operators.yml \
