...
shield archives --tenant master-depls --target shieldv8-192.168.99.26 --limit 1
ARCHIVE_UUID=$(shield archives --tenant master-depls --target shieldv8-192.168.99.26 --limit 1 | grep "bzip2" | grep -oE '[0-9a-f]{1,8}' | head -n 1)
ARCHIVE=$(shield archives --tenant master-depls --target shieldv8-192.168.99.26 --limit 1 | grep "bzip2" | grep -oE '[\/.][0-9]{1,4}[\/.][0-9]{1,2}[/\.][0-9]{1,2}[/\.][0-9]{1,4}-[0-9]{1,2}-[0-9]{1,2}-[0-9]{1,6}-[0-9a-f]{1,8}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,12}' | head -n 1)
ARCHIVE_FILE=$(shield archives --tenant master-depls --target shieldv8-192.168.99.26 --limit 1 | grep "bzip2" | grep -oE '[0-9]{1,4}-[0-9]{1,2}-[0-9]{1,2}-[0-9]{1,6}-[0-9a-f]{1,8}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,12}' | head -n 1)
echo "ARCHIVE_UUID : ${ARCHIVE_UUID}"
echo "ARCHIVE : ${ARCHIVE}"
echo "ARCHIVE_FILE : ${ARCHIVE_FILE}"
shield archive --tenant master-depls ${ARCHIVE_UUID}
FULL_UUID=$(shield archive --tenant master-depls ${ARCHIVE_UUID} | grep "UUID" | grep -oE '[0-9a-f]{1,8}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,12}' | head -n 1)
...
mc cp shield_local/${BUCKET_PREFIX}-shieldv8${ARCHIVE} .
