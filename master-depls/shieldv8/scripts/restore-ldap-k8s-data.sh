...
#USAGE#
#This script restores on shield VM the latest ldap k8s valid archive from a source target given as parameter, fs plugin)
...
#Ask the operator to connect to the shield VM and to execute the generated script (bash decrypt.sh)
#The uncrypted and uncompressed archive (called file) will be available under /tmp/work
#The uncrypted and uncompressed archive must be downloaded on the docker bosh cli
#[VM]chmod 755 <file>
#[BOSH-CLI]bosh scp shield:/tmp/work/<file> .
#The uncrypted and uncompressed archive must be uploaded to the target agent
#bosh scp <file> <target-agent>:/var/vcap/store/k3s-local-storage-path/pvc-<guid>_openldap_ldap-data-ldap-0/.

#LAUNCH# restore-ldap-k8s-data.sh 01-ci-k8s-ldap-192.168.116.240

if [ "$1" != "" ]; then
  source_system=$1 #01-ci-k8s-ldap-192.168.116.240
else
	echo "source target is required as input => restore-ldap-k8s-data.sh 01-ci-k8s-ldap-192.168.116.240"
  exit 1
fi
...
shield archives --tenant micro-depls --target ${source_system} --limit 1
ARCHIVE_UUID=$(shield archives --tenant micro-depls --target ${source_system} --limit 1 | grep "bzip2" | grep -oE '[0-9a-f]{1,8}' | head -n 1)
ARCHIVE=$(shield archives --tenant micro-depls --target ${source_system} --limit 1 | grep "bzip2" | grep -oE '[\/.][0-9]{1,4}[\/.][0-9]{1,2}[/\.][0-9]{1,2}[/\.][0-9]{1,4}-[0-9]{1,2}-[0-9]{1,2}-[0-9]{1,6}-[0-9a-f]{1,8}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,12}' | head -n 1)
ARCHIVE_FILE=$(shield archives --tenant micro-depls --target ${source_system} --limit 1 | grep "bzip2" | grep -oE '[0-9]{1,4}-[0-9]{1,2}-[0-9]{1,2}-[0-9]{1,6}-[0-9a-f]{1,8}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,4}-[0-9a-f]{1,12}' | head -n 1)
...
mc cp shield_local/${BUCKET_PREFIX}-01-ci-k8s${ARCHIVE} .
...
. /usr/local/bin/log-bosh.sh -t "master" -d "shieldv8"
