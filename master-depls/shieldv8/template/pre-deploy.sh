...
for tenant in $(echo ${TENANTS} | tr "|" " "); do
    ...
    for deployment in $(echo ${DEPLOYMENTS} | tr "|" " "); do
        echo ${deployment}
        cp ${GENERATE_DIR}/09-add-shield-import-system-bbr-deployment-${tenant}-depls-errand-operators.yml ${GENERATE_DIR}/09-add-shield-import-system-bbr-deployment-${deployment}-errand-operators.yml
        sed -i -e "s/#deployment#/${deployment}/g" ${GENERATE_DIR}/09-add-shield-import-system-bbr-deployment-${deployment}-errand-operators.yml
    done
